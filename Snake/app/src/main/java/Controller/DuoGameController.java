...
public class DuoGameController {
    ...
    public void run(){
        ...
        if(isHitting(player1) || isHitting(player2)) {
            try {
                ...
                stage.show();
                gdc.setInitialScore(this.getWinner(), this.playerOneScore.getText(), this.playerTwoScore.getText());
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public String getWinner(){
        String hasil = "Both";
        int s1 = Integer.parseInt(this.playerOneScore.getText());
        int s2 = Integer.parseInt(this.playerTwoScore.getText());

        if(isHitting(player2)){
            hasil = "Player 1";
        }
        else if (isHitting(player1)){
            hasil = "Player 2";
        }
